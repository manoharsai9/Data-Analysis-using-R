...
# Task 2: Creating a histogram which represents the Price variable. Using the default binwidth values first and then set bins of length $100,000. Using the data on all apartments, no matter if sold or not.
# Task 3: Interpreting the results.
# Task 4: Creating a scatter plot showing the relationship between Price and Area. Using the data on all apartments, no matter if sold or not. Interpreting the results.
# Task 5: Calculating the mean, median, mode, skewness, and standard deviation of Price for all apartments, no matter if sold or not.
# Task 6: Interpreting the measures.
# Task 7: Calculating the correlation between Price and Area. Is the result in line with the scatterplot?
...
# Year of sale - Numerical, discrete/Interval.	Year is a numerical variable. It is always discrete. The level of measurement is questionable, but I would treat it as interval, as the 0 year would be the time when the Big Bang happened. The current BC-AD calendar was arbitrary chosen (similarly to degrees Celsius and Fahrenheit).
...
# I will only comment on the skew, as it is a bit tougher.
